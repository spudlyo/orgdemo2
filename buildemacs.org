...
it, and some other development tools, using the =dnf= package management tool. Here we provide the =-y= option so =dnf=
doesn't ask us any questions, and we'll use the =--quiet= argument to suppress the copious amount of output that =dnf=
normally spits out.

#+caption: Install the the Development Tools group of packages
#+name: rh_install_git
#+begin_src bash :dir ~/
  sudo dnf groupinstall -y --quiet git
#+end_src
