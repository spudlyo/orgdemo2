...
** Running configure                                                 :debian:
Let's see what happens when we run =./configure= for the first time. We'll provide our =--prefix= argument and we'll
also redirect STDERR to STDOUT so all the output goes to the same place. Finally we'll take advantage of the =bash=
logical OR (=||=) short-circuit to echo a message if =configure= returns a non-zero exit code.
...
#+RESULTS[89c7b86937fc395d7e266087ed1a997523114667]: configure_first
#+begin_example
...
Let's see what happens when we run =./configure= again. We'll provide our =--prefix= argument as per usual, and we'll
also redirect STDERR to STDOUT so all the output goes to the same place. Since the =configure= script can output quite a
large volume of text to the terminal we'll pipe it into =tail= to just see the last handful of lines. Finally we'll
utilize the =bash= array variable =PIPESTATUS= (which holds the exit statuses of each command executed in the most
recent pipeline) to determine if the script invocation failed, and if does fail we'll =echo= something.

#+caption: Our first attempt at running configure
#+name: configure_first
#+begin_src bash
  ./configure --prefix=<<tag_dir()>> 2>&1 | tail -5; [[ ${PIPESTATUS[0]} -ne 0 ]] && echo Configure failed!
#+end_src

#+RESULTS[600f149f668bc4edcff11c2bb59a08a950ba1aba]: configure_first
#+begin_example
...
GNU project, and Emacs creates =info= files for the wealth of documentation that comes with it. By default on
{{{pr(osver)}}} none of the GNU Texinfo tools (like =makeinfo=) are installed. Luckily we can install them ourselves!

#+caption: Install the GNU Texinfo tools
#+name: install_texinfo
#+begin_src bash
  echo <<get_prop("install")>> texinfo >/dev/null 2>&1
#+end_src
