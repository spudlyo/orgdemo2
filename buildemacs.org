...
** Installing git :redhat:
By default, on the Fedora systems I tested on, the =git= tool is not installed, so the first thing we'll do is install
it using the =dnf= package management tool. Here we provide the =-y= option so =dnf= doesn't ask us any questions, and
we'll use the =--quiet= argument to suppress the copious amount of output that =dnf= normally spits out.

#+caption: Install the git package
#+name: rh_install_git
#+begin_src bash :dir ~/
  sudo dnf install -y --quiet git
#+end_src

#+RESULTS[0226e54065a340234927600b9187cd30d9ae9c4e]: rh_install_git
#+begin_example

Installed:
  git-2.41.0-1.fc38.aarch64            git-core-2.41.0-1.fc38.aarch64          
  git-core-doc-2.41.0-1.fc38.noarch    perl-Error-1:0.17029-11.fc38.noarch     
  perl-Git-2.41.0-1.fc38.noarch        perl-TermReadKey-2.38-16.fc38.aarch64   
  perl-lib-0.65-497.fc38.aarch64      
...
#+RESULTS[9ad8416d923624d8c4e12a96d06af904c4b71b6d]: create_src_dir
...
#+RESULTS[9cee994c337fcfab0672e687b456621eefdfaffc]:
...
#+RESULTS[c2efae208543a8f8e8644df957aa5644f30946ee]: num_tags
#+begin_example
     184
#+end_example

Wow {{{r(num_tags)}}} tags is a lot! We can narrow it down a bit by listing just the tags for Emacs 29. We'll use the
=git tag= command with the =--list= argument (using the shell wildcard syntax) as well as specificying the =--sort= option
...
#+caption: List all the tags for Emacs version 29 sorted by date in decending tagged date order
#+name: sorted_tags
#+begin_src bash
  git tag --list 'emacs-29.*' --sort=-taggerdate -n1
#+end_src

#+RESULTS[8fdd872125ec58e33cba350c8016bf3f6bbf4c0c]: sorted_tags
#+begin_example
...
#+RESULTS[fb4c714352ffd54d2a61f1f04b4411ebe83a2b39]: git_branch
#+begin_example
master
#+end_example
...
#+RESULTS[d6f3ff140c09067d55391d42b80a474e39409954]: create_branch
#+begin_example
...
:header-args:bash:  :results output :dir ~/src/emacs
:END:
If you've ever built an old-school UNIX program from source, you've probably run the =./configure= script. This script
...
trivial progrmas you can run an incantation like =./configure && make && make install= and you're good to go.
...
#+RESULTS[bab83ce9cb4a0badb8e071311cea3aa80fd3e4e3]: where_configure
#+begin_example
configure.ac
#+end_example

Looks like there isn't a =./configure= script for us to run, so we'll have to make one.
