...
By default, on the Fedora systems I tested on, the =git= tool is not installed, so the first thing we'll do is install it
using the =dnf= package management tool. Here we provide the =-y= option so =dnf= doesn't ask us any questions.
...
Now we just got to procure some source code. We'll do this using =git=
