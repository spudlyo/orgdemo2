...
trivial progrmas you can run an incantation like =./configure && make && make install= and you're good to go. Let's see
if such a script exists in the Emacs repo.
...
file as well as adding various hooks in =.git/hooks= that are suitable for Emacs development.

Let's run the script, and take a look at the first few lines of the output. We'll run =autogen.sh= with the =autoconf=
argument to tell it to only generate the =configure= script, and to leave our git configuration untouched.
...
Now if we run the =autogen.sh= script again it should generate our =configure= script.
...
#+RESULTS[3a2e7f1f8060e8db336b965dbdc4b6b96f561a0e]: autogen_redux
#+begin_example
Checking whether you have the necessary tools...
(Read INSTALL.REPO for more details on building Emacs)
