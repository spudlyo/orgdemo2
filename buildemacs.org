...
** Emacs Prerequisites
This document has been tested on Emacs 27.1, Emacs 28.2, and Emacs 29.1. It may work on earlier versions of Emacs if
...
/without/ a password, you'll need to make a change to the =/etc/sudoers= file with the =sudo visudo= command.

You should see a section in the file that allows users of a specific group (in this case the =sudo= group) to execute any
command.

#+CAPTION: Relevant section from /etc/sudoers
#+begin_example
...
#+CAPTION: Updated section from /etc/sudoers
#+begin_example
# Allow members of group sudo to execute any command
