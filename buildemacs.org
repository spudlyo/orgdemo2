...
#+begin_src bash
  <<get_prop("install")>> build-essential pkg-config >/dev/null 2>&1
#+end_src
...
Now you might be asking yourself, "How do you know that the =makeinfo= tool is in the =texinfo= package?". It might not
be obvious to you that =makeinfo= is part of the GNU Texinfo system, and if you didn't know that you'd likely struggle
to figure out the name of the package to install to get the =makeinfo= program that the =configure= script is
complaining about!

*** Determining the package name :debian:
Luckily there is a handy tool called =apt-file= that you can install that can help you search packages looking for
specific files.

#+caption: Install apt-file and update the database
#+name: install_apt_file
#+begin_src bash
  <<get_prop("install")>> apt-file >/dev/null 2>&1
...
the =--regex= option to use a regular expression search for files that end in =makeinfo=.
...
*** Determining the package name :redhat:
The =dnf= tool has a =provides= subcommand that you can use to determine which package a given file exists in. We can
use this to determine which package that the =makeinfo= command is in.
